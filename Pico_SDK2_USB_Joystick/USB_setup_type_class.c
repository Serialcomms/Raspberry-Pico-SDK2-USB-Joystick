...
void usb_setup_type_class() {
    ...
    switch (setup->recipient) {
        ...
        case USB_REQ_TYPE_RECIPIENT_INTERFACE:      usb_setup_hid_joystick();       break;      // HID set idle
...
static inline void usb_setup_class_unknown() {
    ...
    printf("Setup Packet IRQ unknown recipient=%d, type=%d, direction=%d, request=%d, value=%d, index=%d, length=%d \n\r", 
