...
void ep0_handler_to_host(uint8_t EP_NUMBER) {
  ...
  printf("EP%d handler to Host, clearing buffer status, buffer bytes=%d, queue level=%d, last PID=%d\n\r", 
...
static inline void ep0_queue_runner_to_host() {
  ...
  printf("Continue Queued Transfer, data count = %d\n\r", source_data_offset);
...
void ep0_handler_to_pico(uint8_t EP_NUMBER) {
  ...
  printf("EP%d handler to Pico, clearing buffer status, buffer bytes=%d, queue level=%d, last PID=%d\n\r", 
  0, 
