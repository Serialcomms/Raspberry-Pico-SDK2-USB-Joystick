...
#include "include/USB_buffer_status.h"
...
static inline void usb_irq_bus_reset() {
  ...
  printf("USB Bus Reset command received from host, function=%s\n\r", __func__);
...
static inline void usb_irq_unhandled() {
  ...
  printf(SEPARATOR);
  printf("USB Unhandled IRQ received, IRQ=%08x, buffer=%08X, SIE=%08X\n\r", 
...
void enable_setup_interrupts() {
  ...
  printf("Enabling Setup Request Interrupts\n\r");
...
void disable_setup_interrupts() {
  ...
  printf("Disabling Setup Request Interrupts\n\r");
...
void enable_buffer_interrupts() {
  ...
  printf("Enabling Buffer Completion Interrupts\n\r");
...
void disable_buffer_interrupts() {
  ...
  printf("Disabling Buffer Completion Interrupts\n\r");
