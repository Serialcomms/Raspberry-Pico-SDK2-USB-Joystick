...
void start_queue_transfer_to_host(uint8_t EP_NUMBER, const uint8_t *source_data, uint16_t source_data_length) {
  ...
  uint16_t queue_free_length = get_queue_free_to_host(EP_NUMBER);

  if (source_data_length > queue_free_length) {

    printf("ERROR - insufficient transmit queue space, required=%d, available=%d\n\r", 
...
static inline void prepare_queue_transfer_to_host(uint8_t EP_NUMBER, const uint8_t *source_data, uint16_t source_data_length) {
  ...
  if (queue_busy) {
    ...
    extend_queue_transfer_to_host(EP_NUMBER, source_data, source_data_length);  
...
static inline void extend_queue_transfer_to_host(uint8_t EP_NUMBER, const uint8_t *source_data, uint16_t source_data_length) {

  uint16_t queue_free_length = get_queue_free_to_host(EP_NUMBER);

  switch (host_endpoint[EP_NUMBER].transfer_type) {

    case USB_TRANSFER_TYPE_BULK: 

      if (source_data_length > queue_free_length) {

        printf("ERROR - insufficient transmit queue space, required=%d, available=%d\n\r", source_data_length, queue_free_length);
    ...
    default:
      ...
      printf("ERROR - Cannot extend active transfer queue for Endpoint Type=%d\n\r", host_endpoint[EP_NUMBER].transfer_type);
