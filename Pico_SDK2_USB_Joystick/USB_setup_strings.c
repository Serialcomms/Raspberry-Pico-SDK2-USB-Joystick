...
const uint8_t *pico_string_descriptors[] = {
    ...
    [2] = "Pico HID Joystick",          // Product
    [3] = "Pico HID Interface",         // Interface
...
static inline void send_text_string(uint8_t string_index) {
    ...
    if (string_index < count_of(pico_string_descriptors)) {
        ...
        printf("Sending Text String, Index=%d, Length=%d, String=%s\n\r", 
        string_index, string_length, pico_string_descriptors[string_index]);

        start_control_transfer_to_host(string_buffer, string_length);  
      
    } else {

        printf("String index %d out of bounds\n\r", string_index);

        send_zlp_to_host(0);        // todo - check for new PID / / String index 238 out of bounds
...
static inline void send_language_string() {
    ...
    printf("Sending Language String, Index=%d, bytes=%02x, %02x\n\r", 
    0, language[0], language[1]);
...
static inline uint8_t usb_convert_string_descriptor(uint8_t string_index) {
    ...
    printf("String index=%d, string length=%d, buffer length=%d\n\r", string_index, string_length, buffer_length) ;
